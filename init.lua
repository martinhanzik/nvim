...
do
  ...
  if vim.env.ZELLIJ or vim.env.SSH_TTY then
    -- Zellij eats nvim's terminal capability query, so force the OSC 52 provider.
  ...
  vim.api.nvim_create_autocmd('TextYankPost', {
    ...
    callback = function()
      vim.hl.on_yank()
      if vim.g.clipboard and vim.g.clipboard.name == 'OSC 52' and vim.v.event.operator == 'y' then
        vim.g.clipboard.copy['+'](vim.v.event.regcontents)
      end
    end,
  })
end
